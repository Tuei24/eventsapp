              ...
              <CardContent sx={{ p: 3 }}>
                ...
                {eventsToday.map((event, index) => (
                  <Box key={index} display="flex" alignItems="flex-start" sx={{ flexDirection: { xs: 'column', sm: 'row' }, gap: { xs: 2, sm: 0 } }} mb={2}>
                    <Typography variant="body2" color="#667eea" fontSize="0.875rem" fontWeight="600" px={2} pt={0.75}>{event.time}</Typography>
                    <Box sx={{ flex: 1 }}>
                ...
                {upcomingEvents.map((event, index) => (
                  <Box key={index} display="flex" alignItems="center" sx={{ flexDirection: { xs: 'column', sm: 'row' }, gap: '1rem', padding: '0.75rem', backgroundColor: '#f8fafc', borderRadius: '0.5rem' }} mb={2}>
                    <Box sx={{ background: 'linear-gradient(135deg, #667eea, #764ba2 100%)', color: 'white', borderRadius: '0.5rem', padding: '0.5rem', textAlign: 'center', minWidth: '50px', mr: 2 }}>
                      <Typography variant="subtitle4" fontWeight={600}>{event.date}</Typography>
                    </Box>
